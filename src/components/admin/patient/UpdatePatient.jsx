...
const UpdatePatientModal = ({ open, handleClose, patient }) => {
  // Custom hook to call update API
  const update = useUpdate();
  ...
  useEffect(() => {
    if (patient && typeof patient === "object") {
      Object.entries(patient).forEach(([key, value]) => {
        if (value !== undefined && value !== null) {
          if (key === "dateOfBirth" && typeof value === "string") {
            setValue(key, value.slice(0, 10)); // format YYYY-MM-DD
          } else {
            setValue(key, value);
          }
        }
  ...
  // Mutation hook to update patient
  const { mutate } = useMutation(updatePatient, {
    onSuccess: () => {
      queryClient.invalidateQueries(["patients"]);
      reset({});
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-center min-h-screen px-2 sm:px-4">
        <div className="relative w-full max-w-sm sm:max-w-md md:max-w-lg lg:max-w-2xl xl:max-w-3xl bg-gray-50 dark:bg-gray-800 rounded-xl shadow-lg p-4 sm:p-6 overflow-y-auto max-h-[90vh]">
          {/* Header */}
          <div className="flex justify-between items-center pb-4 mb-4 border-b dark:border-gray-600">
            ...
            <button
              type="button"
              onClick={handleClose}
              className="text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-600 hover:text-gray-900 dark:hover:text-white rounded-lg p-2"
            >
              <svg
                aria-hidden="true"
                className="w-5 h-5"
          ...
          {/* Form */}
          <form onSubmit={handleSubmit(handlePatientUpdate)}>
            <div className="grid gap-4 mb-6 grid-cols-1 sm:grid-cols-2">
              ...
              <div>
                ...
                <input
                  ...
                  {...register("firstname")}
                  type="text"
                  className="w-full p-2.5 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-sm text-gray-900 dark:text-white focus:ring-2 focus:ring-primary-500 focus:border-primary-500"
                />
                {errors.firstname && (
              ...
              {/* Add other input fields here (lastname, email, etc.) */}
            </div>

            {/* Submit button */}
            <button
              type="submit"
              className="w-full md:w-auto flex items-center justify-center gap-2 bg-primary-700 hover:bg-primary-800 dark:bg-primary-600 dark:hover:bg-primary-700 text-white font-semibold rounded-lg px-5 py-2.5 text-sm md:text-base focus:ring-4 focus:outline-none focus:ring-primary-300 dark:focus:ring-primary-800"
            >
