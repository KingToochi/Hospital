...
const Login = () => {
    ...
    return (
        <div className='bg-sky-50 flex flex-col h-auto gap-[5rem] items-center w-[50%] m-auto'>
            ...
            <form className="flex flex-col gap-[3rem] w-full mb-10" onSubmit={handleSubmit(login)}>
                ...
                <button className='border-2 rounded-[10px] w-[20%] m-auto bg-white cursor-pointer' type='submit'>
                    {loading ? "Loggin in..." : "Login"}

                </button>
                <label className='flex ml-[50px]'>
                    <input type="checkbox" checked="defaultChecked" name="remember" /> Remember me
                    <span className="ml-[250px]">Forgot <Link href="#">password?</Link></span>
                </label>
            </form>
        </div>
     );
